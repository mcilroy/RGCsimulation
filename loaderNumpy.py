...
def load_MITFaces(dataset="training",path=None,asbytes=True,selection=None,return_labels=True):
    files = {
        'training': ('svm.train.normgrey'),
        'testing': ('svm.test.normgrey'),
    }
    try:
        images_fname = os.path.join(path, files[dataset])
    except KeyError:
        raise ValueError("Data set must be 'testing' or 'training'")
##    f = open(images_fname, 'r')
##    raw_images = []
...
##    f.close()
    ...
    with open(images_fname, 'rb') as csvfile:
        reader = csv.reader(csvfile, delimiter=' ', quotechar='|',skipinitialspace = True)
        for i,line in enumerate(reader):
            if i == 0:
                numExamples = int(line[0])
                indices = range(numExamples)
                if selection:
                    indices = indices[selection] 
                N = len(indices)
            elif i == 1:
                pixelSize = int(line[0])
                rows,cols=sqrt(pixelSize),sqrt(pixelSize)
                raw_images = zeros((N, rows, cols), dtype=float32) # my edit from uint8 to float32 - Stu
                raw_labels = zeros((N), dtype=float32) # my edit - Stu
            else:
                if i+2 < N:
                    raw_labels[i-2] = float(line.pop())
                    if asbytes:
                        #line = line.astype(float)*255.0
                        line = [float(element)*255.0 for element in line]
                    raw_images[i-2] = array(line).reshape((rows,cols))
                else:
                    break
...
def load_mnist(dataset="training", digits=None, path=None, asbytes=False, selection=None, return_labels=True, return_indices=False):
    ...
    images = zeros((N, rows, cols), dtype=float32) # my edit from uint8 to float32 - Stu

    if return_labels:
        labels = zeros((N), dtype=float32) # my edit - Stu
